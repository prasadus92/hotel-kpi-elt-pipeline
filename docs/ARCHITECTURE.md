...
1. **Extract / Load (Python).** Read the raw Odyssey PMS JSON and land it in
   DuckDB untouched.
...
(`dbt/rpg_kpi/rpg.duckdb`). It is git-ignored and fully regenerated on every run,
so the repository never stores derived state. The path is configurable through
the `RPG_DUCKDB_PATH` environment variable, which `run_pipeline.py` sets so the EL
...
inside the warehouse with set-based SQL. This matches how the real RoomPriceGenie
...
The following is what would change to run this at RoomPriceGenie scale. The dbt
