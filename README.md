# the platform — Daily Hotel Performance KPIs
...
| **Transform** | `dbt/hotel_kpi/` | Casts, validates, deduplicates, explodes nights, filters to inventory, and aggregates KPIs across layered models. | DuckDB schema `main` (views + tables) |
...
(`dbt/hotel_kpi/hotel_kpi.duckdb`, git-ignored, regenerated on every run). Staging and
...
- **ELT, not ETL.** We land the raw JSON first (cheap, replayable, a faithful
  ...
  real the platform stack works, it keeps extraction dumb and idempotent, and
...
What I would change to take this to production at production scale:
